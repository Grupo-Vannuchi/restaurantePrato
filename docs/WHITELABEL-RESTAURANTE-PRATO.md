...
| Cardápio do buffet | **102 pratos** em **quinze listas** — cinco dias × três semanas. O papel cresceu em três entregas: 5 listas em 03/09, 10 em 17/09 e 15 em **21/09/2026**. Entram por `scripts/importa-cardapio.mjs`, que recusa a carga se alguma lista não fechar com a do cliente. ⚠️ A rotação de semana **não cabe no modelo** (`MenuItem.weekdays` é `Int[]` de 1 a 5): o banco recebe a UNIÃO, e a semana fica preservada nos tokens da fonte. ⚠️ **A quarta deixou de rodar em 23/09/2026**: o cliente mandou um quadro novo e o dono decidiu que ele substitui as três semanas, então `3a`, `3b` e `3c` são hoje a mesma lista de 19 pratos. Saíram 9 pratos do site e um mudou de nome — `REMOVIDOS`, no script, tem a lista com motivo, porque `upsert` não apaga nada sozinho |
